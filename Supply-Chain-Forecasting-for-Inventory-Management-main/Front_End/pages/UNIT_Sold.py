...
class UnitSold:
    ...
    def todays_units_sold(self):
        today = date.today().strftime('%Y-%m-%d')
    
        try:
            future_df = self.predict_future(start_date=today, end_date=today)
            predicted_value = int(future_df['Predicted_Units_Sold'].sum())
            return predicted_value
        except Exception as e:
            print(f"Error predicting today's units sold: {e}")
            return 0
