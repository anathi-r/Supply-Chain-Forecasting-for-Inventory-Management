...
class UnitSold:
    ...
    def train_model(self):
        model_path = f"Models/xgb_model_{self.store}.pkl"
        if os.path.exists(model_path):
            print(f"Loading model for {self.store}...")
            self.model = joblib.load(model_path)
            return

        print(f"Training model for {self.store}...")
        data = self.df.copy()
    ...
    def evaluate_model(self, Y_test, predictions):
        print("Model Evaluation (XGBoost Model):")
    ...
    def predict_future(self, start_date=None, end_date=None, past_year=2023):
        ...
        return int(future_df["Predicted_Units_Sold"].iloc[0])